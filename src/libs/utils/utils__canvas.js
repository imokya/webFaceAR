...
export const drawFillCurvedTriangles2 = (ctx, vertices, triangles, color, alpha) => {

  ctx.strokeStyle           = getColor(color, alpha)
  ctx.fillStyle             = getColor(color, alpha)
  ctx.lineWidth             = 0.28

  const st = triangles[0]
  const sx = vertices[st * 2]
  const sy = vertices[st * 2 + 1]
  ctx.beginPath()
  ctx.moveTo(sx, sy)
  let cx, cy, nx, ny
  for(let i = 1; i < triangles.length; i += 1) {
    const t = triangles[i]
    const x = vertices[t * 2]
    const y = vertices[t * 2 + 1]
    ctx.lineTo(x, y)
  }
  ctx.lineTo(sx, sy)
  ctx.fill()
...
export const drawFillCurvedTriangles = (ctx, vertices, triangles, color, alpha) => {
  ...
  ctx.moveTo(sx, sy)
  let cx, cy, nx, ny
  for(let i = 1; i < triangles.length - 1; i += 1) {
    const t = triangles[i]
    const x = vertices[t * 2]
    const y = vertices[t * 2 + 1]
    const nt = triangles[i　+ 1]
    nx = vertices[nt * 2]
    ny = vertices[nt * 2 + 1]
    cx = (x + nx) / 2
    cy = (y + ny) / 2
  ...
  ctx.quadraticCurveTo(nx, ny, sx, sy)
  ctx.fill()
  ctx.stroke()
}
